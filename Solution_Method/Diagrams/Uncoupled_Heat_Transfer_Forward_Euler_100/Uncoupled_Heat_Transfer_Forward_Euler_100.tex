...
\begin{tikzpicture}
	...
	\addplot[only marks, blue, legend entry=Analytic solution] table[x=time, y=temp_an] {temp_transfer_time_step_100.txt};
	\addplot[only marks, green, legend entry=Numerical solution] table[x=time, y=temp_num] {temp_transfer_time_step_100.txt};
	\end{axis}
\end{tikzpicture}
\end{document}