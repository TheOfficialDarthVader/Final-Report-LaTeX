...
\begin{tikzpicture}[node distance=3cm]
	...
	\node (pro1) [process, below of=start, align=center] {$T_{d_{n}} = T_{d_{0}}$ \\ $t_n = 0$\\$t=0$};
	\node (dec1) [decision, below of=pro1, yshift=-1cm] {$t>\textrm{Max No. timesteps}$};
	\node (dec2) [decision, below of=dec1, yshift=-1.8cm] {$\frac{T_{d_{n}}}{T_G}>0.999$};
	\node (pro2) [process, right of=dec2, xshift=2cm, align=center] {$t_{n+1}=t/t_{div}$\\$\Delta t = t_{n+1}-t_n$\\$T_{d_{n+1}}=T_{d_{n}}+(\Delta t~T_{d_{n}})$\\$T_{d_{n}}=T_{d_{n+1}}$\\$t_{n}=t_{n+1}$\\$t=t+1$};
