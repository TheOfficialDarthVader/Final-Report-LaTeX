...
\begin{tikzpicture}
	...
	xlabel={$\Delta t/\tau_{d}$},
	ylabel={Average Percentage Error}
	]
	\addplot[mark=none, black, legend entry=Linear fit] {5*x};
	\addplot[dashed, legend entry=Error data] table[x=timestep_tau, y=error] {gravity_convergence_data.txt};
	\end{axis}
\end{tikzpicture}
\end{document}